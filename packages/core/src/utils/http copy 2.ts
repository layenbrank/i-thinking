        ...
        if (this.config.queueFullStrategy === "reject-new") {
          ...
          if (
            ...
            const error = new Error(
              "Request dropped due to higher priority request",
            );
            this.logger.warn(
              "Request dropped: replaced by higher priority request",
              {
    ...
    this.instance.interceptors.response.use(
      (response: AxiosResponse) => {
        ...
        return this.responseInterceptors
          .reduce(
            (promise, interceptor) =>
              promise.then((res) => interceptor.intercept(res, this)),
            Promise.resolve(response),
          )
          .then((finalResponse) => {
            // 添加性能监控拦截器
            return new PerformanceMonitorInterceptor().intercept(
              finalResponse,
              this,
            );
          })
