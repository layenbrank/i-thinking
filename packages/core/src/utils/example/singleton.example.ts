 ...
 * 数据库异步生命周期监听器
 */
class DBAsyncLifecycleListener implements AsyncLifecycleListener<DatabaseService> {
  async onAsyncEvent(
    event: 'beforeCreate' | 'created' | 'beforeDestroy' | 'destroyed',
    instance: DatabaseService
  ): Promise<void> {
...
class CustomErrorStrategy implements ErrorHandlingStrategy {
  handleError(error: Error, context: ErrorContext): void {
    ...
    if (context.retryCount < context.maxRetries) {
      console.log(`Retrying (${context.retryCount + 1}/${context.maxRetries})...`)
      return
    }

    throw new Error(`Failed after ${context.maxRetries} retries: ${error.message}`)
  }
}
...
export async function example() {
  try {
    ...
    console.log('DB State after destroy:', SingletonProxy.getState(DatabaseService))

    // 7. 实例重置示例
    console.log('\n=== Instance Reset Example ===')
    SingletonProxy.reset(CacheService)
    console.log('Cache State after reset:', SingletonProxy.getState(CacheService))
  } catch (error) {
