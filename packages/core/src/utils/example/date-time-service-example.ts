...
async function basicExample() {
  ...
  console.log(
    "带毫秒:",
    dateTimeService.format(date, "YYYY-MM-DD HH:mm:ss.SSS"),
  );
...
async function calculationExample() {
  ...
  console.log(
    "一天后:",
    dateTimeService.format(dateTimeService.add(now, 1, "day")),
  );
  console.log(
    "一周后:",
    dateTimeService.format(dateTimeService.add(now, 7, "day")),
  );
  console.log(
    "一个月后:",
    dateTimeService.format(dateTimeService.add(now, 1, "month")),
  );

  // 2. 减少时间
  console.log(
    "一天前:",
    dateTimeService.format(dateTimeService.subtract(now, 1, "day")),
  );
  console.log(
    "一周前:",
    dateTimeService.format(dateTimeService.subtract(now, 7, "day")),
  );
  console.log(
    "一个月前:",
    dateTimeService.format(dateTimeService.subtract(now, 1, "month")),
  );
...
async function workdayExample() {
  ...
  console.log(
    "周一是否工作日:",
    dateTimeService.workday.isWorkday("2025-01-20"),
  ); // true
  console.log(
    "周六是否工作日:",
    dateTimeService.workday.isWorkday("2025-01-19"),
  ); // false
...
async function dateInfoExample() {
  ...
  console.log(
    "日期开始:",
    dateTimeService.format(dateTimeService.date.startOfDay(date)),
  );
  console.log(
    "日期结束:",
    dateTimeService.format(dateTimeService.date.endOfDay(date)),
  );
}
...
async function quarterExample() {
  ...
  console.log(
    "季度开始:",
    dateTimeService.format(dateTimeService.quarter.startOf(date)),
  );
  console.log(
    "季度结束:",
    dateTimeService.format(dateTimeService.quarter.endOf(date)),
  );
...
async function weekExample() {
  ...
  console.log(
    "周开始:",
    dateTimeService.format(dateTimeService.week.startOf(date)),
  );
  console.log(
    "周结束:",
    dateTimeService.format(dateTimeService.week.endOf(date)),
  );
  ...
  console.log(
    "本周所有日期 (中文):",
    getChineseWeekDates(dateTimeService.format(date, "YYYY-MM-DD")).map(
      (d) => ({
  ...
  console.log(
    "本周所有日期 (英文):",
    getEnglishWeekDates(dateTimeService.format(date, "YYYY-MM-DD")).map(
      (d) => ({
        date: dateTimeService.format(d, "MM-DD"),
