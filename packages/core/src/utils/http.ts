...
class RequestEventEmitterInterceptor implements RequestInterceptor {
  ...
  private logRequest(httpClient: HttpClient, eventData: RequestEvent): void {
    httpClient.getLogger().info(`请求开始: ${eventData.method} ${eventData.url}`, {
...
class ResponseEventEmitterInterceptor implements ResponseInterceptor {
  ...
  intercept(response: AxiosResponse, httpClient: HttpClient): any {
    try {
      ...
      // 构建事件数据
      const eventData = this.buildEventData(config, response, endTime, duration)
  ...
  private logResponse(httpClient: HttpClient, eventData: RequestEvent): void {
    ...
    httpClient.getLogger().info(`响应完成: ${method} ${url} ${status} (${duration}ms)`, {
      requestId: eventData.requestId,
      status,
      duration
    })
...
class ErrorEventEmitterInterceptor implements ErrorInterceptor {
  ...
  async intercept(error: any, httpClient: HttpClient): Promise<any> {
    try {
      ...
      // 构建事件数据
      const eventData: RequestEvent = this.buildEventData(config, error, endTime, duration)
   ...
   * @param error - 错误对象
   */
  private logError(httpClient: HttpClient, eventData: RequestEvent, error: any): void {
    ...
    httpClient
      .getLogger()
      .error(`请求失败: ${eventData.method} ${eventData.url}${statusText} - ${errorMessage}`, {
        requestId: eventData.requestId,
...
class CacheResponseInterceptor implements ResponseInterceptor {
  ...
  intercept(response: AxiosResponse, httpClient: HttpClient): any {
    ...
    httpClient
      .getLogger()
      .debug(`Cache stored: ${response.config.method} ${response.config.url} (TTL: ${ttl}ms)`, {
...
class PerformanceMonitorInterceptor implements ResponseInterceptor {
  ...
  private getOriginalResponse(response: any): AxiosResponse | null {
    ...
    if (response && typeof response === 'object' && response.config && response.status) {
      return response
    }
   ...
   * @returns 性能指标对象
   */
  private calculateMetrics(response: AxiosResponse, httpClient: HttpClient): PerformanceMetrics {
    const config = response.config
    const endTime = Date.now()
    const startTime = config.metadata?.startTime || endTime
    const duration = endTime - startTime
   ...
   * @param httpClient - HTTP客户端实例
   */
  private handleSlowRequest(metrics: PerformanceMetrics, httpClient: HttpClient): void {
    httpClient.getEventBus().emit(EventType.SLOW_REQUEST, metrics)
    httpClient.getLogger().warn(`慢请求: ${metrics.method} ${metrics.url} (${metrics.duration}ms)`)
  }
}
      ...
      (response: AxiosResponse) => {
        ...
        return this.responseInterceptors
          .reduce(async (promise, interceptor) => {
            return await promise.then(res => interceptor.intercept(res, this))
