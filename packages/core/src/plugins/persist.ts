...
type StorageType = 'DexieStorage' | 'LocalStorage'
...
abstract class BaseStorage {
  ...
  public storage: Storage | Dexie

  constructor(options: StorageConfig, storage: Storage | Dexie) {
...
export class LocalStorage extends BaseStorage {
  ...
  constructor(options: StorageConfig, storage: Storage) {
...
export class DexieStorage extends BaseStorage {
  public storage: Dexie & {
    store: EntityTable<StoreMetadata, 'key'>
  }

  constructor(options: StorageConfig, storage: Dexie) {
    super(options, storage)
...
async function restoreState(
  ...
  try {
    ...
    const deserialized =
      options.serializer?.deserialize(metadata.value) ?? JSON.parse(metadata.value)
...
async function saveState(
  ...
  try {
    ...
    const omitted = options.omit ? deepOmitUnsafe(picked, options.omit) : picked
    const serialized = options.serializer?.serialize(omitted) ?? JSON.stringify(omitted)
...
export function createPersistedState(context: PiniaPluginContext): void {
  ...
  const persistOptions = Array.isArray(options.persist) ? options.persist : [options.persist]

  persistOptions.forEach(persistOpt => {
    const { storage: storageType, ...storageConfig } = persistOpt
    const storage =
      storageType === 'LocalStorage'
        ? new LocalStorage(storageConfig, window.localStorage)
        : new DexieStorage(storageConfig, new Dexie(storageConfig.key))
    ...
    store.$subscribe(
      async () => {
        await store.$persist()
        const metadata = await store.$store.get({ key: persistOpt.key ?? store.$id })
