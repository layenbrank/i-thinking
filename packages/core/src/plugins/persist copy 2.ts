...
export class LocalStorage extends BaseStorage {
  ...
  public async set(params: Operation) {
    ...
    await localStorage.setItem(params.key, JSON.stringify(params.value))
  }
}
...
// }
function createPersist(config: PersistOptions): (context: PiniaPluginContext) => void {
  const { storage: storageType, ...storageConfig } = config

  const storageMap: Record<string, BaseStorage> = {
    LocalStorage: new LocalStorage(storageConfig),
    DexieStorage: new DexieStorage(storageConfig)
  }

  const storage: BaseStorage = storageMap[storageType]

  return function (context: PiniaPluginContext) {
    const { store, options } = context
    if (!options.persist) return

    // 将 storage 实例的属性和方法赋值给 store.$store
    store.$store = {
