...
export class StorageError extends Error {
  constructor(
    ...
    public code?: 'DISPOSED' | 'INIT_FAILED' | 'VALIDATION_FAILED' | 'MIGRATION_FAILED'
  ) {
    super(message)
...
export class IndexedDBStorage extends BaseStorage implements IBatchOperations {
  ...
  constructor(dbName = 'piniaStore', storageName = 'persistedState', settings: StorageConfig = {}) {
    super()
    ...
    // 异步初始化数据库连接
    this.initComplete = this.initializeStorage(dbName, settings).catch(error => {
      this.storageSettings.onError(
        new StorageError('Failed to initialize storage', error, 'INIT_FAILED')
      )
      return Promise.reject(error)
    })
  }

  private async initializeStorage(dbName: string, settings: StorageConfig): Promise<void> {
    try {
      // 获取或创建新的数据库连接
          ...
          .upgrade(async tx => {
            try {
              ...
              if (updates.length > 0) {
                ...
                console.log(`Migrated ${updates.length} items to version ${targetVersion}`)
              }
            } catch (error) {
  ...
  async batchSet(items: Record<string, string>): Promise<void> {
    ...
    if (this.isShutdown) throw new StorageError('Storage has been disposed', null, 'DISPOSED')

    try {
      // 使用单个事务处理所有操作
      await this.storageDB.transaction('rw', this.getStorageTable(), async () => {
      ...
      this.handleError(error, 'Batch set operation failed', 'VALIDATION_FAILED')
    }
  }
  ...
  async batchGet(keys: string[]): Promise<Record<string, string | null>> {
    ...
    try {
      const items = await this.getStorageTable().where('key').anyOf(keys).toArray()
      ...
      this.storageSettings.onError(new StorageError('Batch get operation failed', error))
    }
  ...
  async batchRemove(keys: string[]): Promise<void> {
    ...
    try {
      ...
      this.storageSettings.onError(new StorageError('Batch remove operation failed', error))
      throw error
    }
  }
  ...
  private async initializeCache(): Promise<void> {
    try {
      ...
      this.storageSettings.onError(new StorageError('Failed to init cache', error, 'INIT_FAILED'))
    }
  }
  ...
  async get(key: string): Promise<string | null> {
    ...
    try {
      ...
      this.storageSettings.onError(new StorageError(`Failed to get item: ${key}`, error))
      return null
    }
  }
  ...
  async set(key: string, value: string): Promise<void> {
    ...
    try {
      ...
      this.handleError(error, `Failed to set item: ${key}`, 'VALIDATION_FAILED')
    }
  }
  ...
  async remove(key: string): Promise<void> {
    ...
    try {
      ...
      this.storageSettings.onError(new StorageError(`Failed to remove item: ${key}`, error))
      if (oldValue !== undefined) {
        this.dataCache.set(key, oldValue)
      }
      throw error
  ...
  async dispose(): Promise<void> {
    ...
    try {
      ...
      this.storageSettings.onError(new StorageError('Failed to dispose storage', error))
      throw error
    }
  }

  private handleError(error: unknown, message: string, code: StorageError['code']) {
...
export function createPersistedState(globalSettings: Partial<PersistOptions> = {}) {
  return function (context: PiniaPluginContext) {
    ...
    persistRules.forEach(rule => {
      const storageOptions = {
        ...
        storage: rule.storage ?? globalSettings.storage ?? new IndexedDBStorage(),
        serializer: rule.serializer ?? globalSettings.serializer ?? defaultSerializer,
      ...
      store.$metadata = async () => {
        const metadata = await storageOptions.storage.getStorageTable().toCollection().first()
        return metadata
      }
