...
| 现象 | 可能原因 | 处理 |
|------|----------|------|
| 启动弹窗：`createRequire` / `filename` Received undefined | Vite 将 main 打成 CJS 时，`import.meta.url` 可能变成 `undefined` | 使用 [`src/plugins/paths.ts`](../../../apps/studio/src/plugins/paths.ts)（`argv[1]` / `APP_ROOT`）；勿在 Main 顶层 `createRequire(import.meta.url)` |
