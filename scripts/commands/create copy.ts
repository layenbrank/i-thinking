...
const TEMPLATE_STRATEGIES = {
  ...
  createWithVite: async (projectPath: string, template: string) => {
    return execCommand(
      `pnpm create vite ${projectPath} --template ${template}`,
      { silent: true },
    );
  },
...
export async function createProject() {
  ...
  try {
    ...
    console.log(`\n${separator}`);
    console.log(
      `${chalk.cyan("🚀 创建项目:")} ${chalk.whiteBright.bold(projectName)}`,
    );
    console.log(
      `${chalk.blue("📦 使用模板:")} ${chalk.whiteBright.bold(template.name)}`,
    );
    ...
    try {
      ...
      console.log(
        `  ${chalk.gray("$")} ${chalk.greenBright(`pnpm dev:${projectName}`)}`,
      );
      console.log(
  ...
  if (existsSync(rootPackageJsonPath)) {
    ...
    try {
      ...
      if (packageJson.scripts) {
        packageJson.scripts[`dev:${projectName}`] =
          `turbo run dev --filter=${projectName}`;
