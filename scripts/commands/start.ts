  ...
  try {
    ...
    if (appsDirectoryExists) {
      ...
      const projectInfos = await Promise.all(
        appDirectories.map(extractProjectInfo),
      );
  ...
  try {
    ...
    if (existsSync(packageJsonPath)) {
      try {
        ...
        console.error(
          chalk.yellowBright(`⚠️ 读取 ${folderName} 的 package.json 失败`),
        );
      }
    }
  } catch (error) {
...
export async function startProject() {
  ...
  try {
    ...
    if (projects.length <= 1) {
      ...
      console.log(
        chalk.yellow("\n提示: ") +
          chalk.greenBright("使用 `pnpm turbo-cli create` 创建新项目\n"),
      );
      return;
    }
    ...
    spinner.succeed(
      chalk.greenBright(
        `✅ 项目扫描完成，找到 ${chalk.white.bold(projects.length - 1)} 个项目`,
      ),
    );
    ...
    console.log(`\n${separator}`);
    console.log(
      `${chalk.cyan("🚀 正在启动:")} ${chalk.whiteBright.bold(selectedProject.name)}`,
    );
    console.log(
      `${chalk.blue("🔧 执行命令:")} ${chalk.gray(selectedProject.command)}`,
    );
