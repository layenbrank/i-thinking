...
export async function createApp(options: OptionValues) {
  try {
    ...
    console.log(`\n${separator}`);
    console.log(
      `${chalk.cyan("🚀 创建项目:")} ${chalk.whiteBright.bold(appName)}`,
    );
    console.log(
      `${chalk.blue("📦 使用模板:")} ${chalk.whiteBright.bold(templateName)}`,
    );
    ...
    try {
      ...
      // templateName
      console.log(
        `${chalk.yellowBright("使用模板:")} ${chalk.whiteBright.bold(
          templateName,
        )}`,
      );
      // 使用选定的模板设置项目
      ...
      console.log(
        `  ${chalk.gray("$")} ${chalk.greenBright(`pnpm dev:${appName}`)}`,
      );
    } catch (error) {
