...
import MarketplaceApplication from './marketplace-magnetic-tile.vue'
import MarketplaceCustomization from './marketplace-customization.vue'
import MarketplaceGame from './marketplace-game.vue'

type ReflectComponent = 'magneticTile' | 'game' | 'ai' | 'customization'
...
const { MAGNETIC_TILES } = useMirror()

const activeKey = ref<GeneralOptions>({
  label: t('General.MagneticTile'),
  key: 'magneticTile'
})
...
const reflect: Record<ReflectComponent, Component> = {
  ...
  magneticTile: MarketplaceApplication,
    ...
    <div class="marketplace-content">
      <template v-for="option in options">
        <component
          ...
          :magneticTiles="MAGNETIC_TILES"></component>
      </template>
    </div>
  </div>
</template>
