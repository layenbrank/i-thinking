...
async function handleExport() {
  ...
  const magneticTiles = await database.magneticTile.orderBy('index').toArray()

  const stringified = JSON.stringify(magneticTiles, null, 2)

  const blob = new Blob([stringified], { type: 'application/json' })
  const url = URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = `magneticTiles-backup-${timeSphere.now().format('YYYY年MM月DD日-HH时mm分ss秒')}.json`
