...
import type { ComboboxTriggerProps, ComboboxTriggerSlots, InputSlotNames } from './index.ts'
...
const filteredSlots = computed(() => {
  ...
  return Object.keys(slots).filter(
    (name): name is InputSlotNames => name !== 'content' && name in slots
  )
})
...
<template>
  <div :class="[clsx('combobox-trigger', comboboxClass)]">
    <n-input v-bind="{ ...props.inputProps, ...attrs }" :class="[clsx(inputClass)]">
