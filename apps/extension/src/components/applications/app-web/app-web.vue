...
function handleJumpLink() {
  ...
  if (!props.slideApp.url) return handleMessage.error('请先设置链接地址!')

  window.open(props.slideApp.url, '_blank')
}
</script>
...
<template>
  <div
    ...
    :data-id="slideApp.id"
    :class="['app-web', slideApp.size, slideApp.shape, slideApp.direction]"
  >
    <AppIcon
      :mini="mini"
      :small="small"
