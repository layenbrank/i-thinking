...
onMounted(() => {
  ...
  nextTick(() => {
    ...
    const throttleDelay = 100 // 增加节流延迟，减少更新频率
    ...
    function applyScaling(mouseX: number): void {
      ...
      items.forEach((item, index) => {
        ...
        // 计算缩放值，保留两位小数减少微小变化
        const scale = parseFloat(scaleCurve(itemCenterX).toFixed(6))
        ...
        if (index < newGapScales.length) {
          ...
          if (index + 1 < items.length) {
            ...
            nextScale = parseFloat(scaleCurve(nextCenterX).toFixed(6))
          }

          const gapScale = parseFloat(((scale + nextScale) / 2).toFixed(6))
      ...
      // 批量更新响应式数据
      newIconScales.forEach((item, index) => {
        iconScales[index].scale = item.scale
      })

      newGapScales.forEach((item, index) => {
        gapScales[index].scale = item.scale
      })
    }
    ...
    function resetScaling(): void {
      ...
      // 重置所有缩放值
      iconScales.forEach((item, index) => {
        item.scale = 1
      })

      gapScales.forEach((gap, index) => {
        gap.scale = 1
      })
    }
