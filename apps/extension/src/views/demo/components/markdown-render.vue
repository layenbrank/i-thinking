...
const TokenMap: Record<MarkedToken['type'], RendererFn<any>> = {
	...
	checkbox(token: Tokens.Checkbox) {
		return <input type="checkbox" checked={token.checked} />
	},
	hr(token: Tokens.Hr) {
	...
	link(token: Tokens.Link) {
		return (
			<a href={token.href} title={token.title ?? undefined} target="_blank">
				{token.text}
			</a>
		)
	},
...
<template>
	<component
		...
		:is="TokenMap[token.type as keyof typeof TokenMap]?.(token)"
	></component>
</template>
