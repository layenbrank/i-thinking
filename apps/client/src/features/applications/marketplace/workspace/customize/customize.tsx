...
import { useMirrorStore, type ApplicationWrite } from '@/stores/mirror.ts'
...
import styles from '@/features/applications/marketplace/workspace/customize/customize.module.scss'
...
function parseImportedWrite(item: Application, mirrorID: string, index: number): ApplicationWrite {
  return {
...
export default function Customize() {
  ...
  const toInsertApplication = useMirrorStore((state) => state.toInsertApplication)
  ...
  async function handleFinish(value: CustomizeFormValues) {
    ...
    try {
      const write = parseCustomizeWrite(value, images, mirrorID, applications.length)
  ...
  const handleImport = useCallback(
    function (file: RcFile, entries: RcFile[]) {
      ...
      const baseIndex = applications.length
      ...
      reader.addEventListener(
        ...
        function () {
          ...
          try {
            ...
            void toInsertApplication(writes).then(
              function () {
                message.success(`已导入 ${writes.length} 个应用`)
              },
              function (error) {
    ...
    [applications.length, mirror?.id, toInsertApplication]
  )
