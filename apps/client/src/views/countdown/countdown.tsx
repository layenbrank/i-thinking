...
import { Button, Checkbox, InputNumber, TimePicker, Tooltip, message } from 'antd'
...
export default function CountdownView() {
  ...
  useEffect(
    function () {
      const h = BASE_HEIGHT + (showSalary ? SALARY_DELTA : 0) + (settingsOpen ? SETTINGS_DELTA : 0)
      getCurrentWindow()
  ...
  useEffect(function () {
    ...
    function sync() {
      ...
      frame = requestAnimationFrame(function () {
        ...
        const nodes = root.querySelectorAll<HTMLElement>('[data-clickable="1"]')
    ...
    function observeAll() {
      root!.querySelectorAll<HTMLElement>('[data-clickable="1"]').forEach(function (el) {
        if (!observed.has(el)) {
          ro.observe(el)
          observed.add(el)
        }
    ...
    <div
      ...
      <div
        ...
        <Tooltip title="工作配置">
          <motion.button
            aria-label="工作配置"
            className={clsx(styles.iconBtn, { [styles.iconBtnActive]: settingsOpen })}
            whileTap={{ scale: 0.9 }}
            onClick={function () {
              setSettingsOpen(function (o) {
                return !o
              })
            }}>
            <Icon
