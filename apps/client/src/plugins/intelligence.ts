...
const IntelligencePlugin: Plugin = {
  ...
  mount() {
    ...
    //       console.error('[IntelligencePlugin] Failed to sync AI sessions:', error)
    //       // 返回空数组以避免应用崩溃，但错误已记录
    ...
    //       return from(
    //         liveQuery(function () {
    //           return database.AiMessage.where('sessionID').equals(sessionID).sortBy('updatedAt')
    ...
    //     }),
    //     catchError(function (error) {
    //       console.error('[IntelligencePlugin] Failed to sync AI messages:', error)
    //       // 返回空数组以避免应用崩溃，但错误已记录
    //       return from(Promise.resolve([]))
    //     })
    //   )
