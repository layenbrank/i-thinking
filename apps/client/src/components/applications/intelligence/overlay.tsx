		...
		function () {
			return sessions.map(function (value) {
				const item: BubbleItemType = {
					...
					content: <XMarkdown content={value.fragment} components={{ code: Code }} />,
					avatar:
						value.identity === 'user' ? (
							<UserOutlined />
						) : (
				...
				<Card className={clsx(styles.provider, styles.card)}>
					<XProvider direction="ltr">
						<Flex gap={12} className={clsx(styles.provider, styles.flex)}>
							...
							<Flex vertical justify="space-between" gap={16} style={{ flex: 1 }}>
								<Bubble.List items={bubbles} rootClassName={clsx(styles.bubble, styles.root)} />
