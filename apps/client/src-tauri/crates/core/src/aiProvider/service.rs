...
use thinking_database::entity::aiProvider as schema;
use uuid::Uuid;
...
impl Service {
    ...
    async fn update_one<C: ConnectionTrait>(
        ...
        let now = Utc::now().timestamp_millis();
        if let Some(v) = payload.change.kind {
            active.kind = Set(v);
        }
        if let Some(v) = payload.change.name {
            active.name = Set(v);
        }
        if let Some(v) = payload.change.baseUrl {
            active.baseUrl = Set(v);
        }
        if let Some(v) = payload.change.models {
            active.models = Set(v);
        }
        if let Some(v) = payload.change.model {
            active.model = Set(v);
        }
