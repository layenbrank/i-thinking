...
| 现象 | 可能原因 | 处理 |
|------|----------|------|
| 启动弹窗：`createRequire` / `filename` Received undefined | Vite 将 main 打成 CJS，`import.meta.url` 变成 `undefined` | 使用 [`src/main/paths.ts`](../src/main/paths.ts)；勿在 Main 顶层 `createRequire(import.meta.url)` |
...
| `bin not allowed` / `bin not found` | 不在白名单或资源未打包 | 查 `ALLOWED_BINS`；确认 `resources/bin` |
...
正确做法：`findAppRequire()` / `findBundleDir()`（基于 `process.argv[1]` 与 `APP_ROOT`）。窗口 preload 路径同样用 `findBundleDir()`，不要 `fileURLToPath(import.meta.url)`。
