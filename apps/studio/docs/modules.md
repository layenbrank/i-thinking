...
| database   | `src/plugins/database.ts`                                  | better-sqlite3 + Drizzle 连接、迁移采纳、User 仓储 IPC |
| chat       | `src/plugins/chat.ts`                                      | chat 域仓储 IPC（provider / session / message）        |
| assistant  | `src/plugins/assistant.ts` + `assistant-{protocol,key}.ts` | 离线通路：MessagePort 流式 + safeStorage 密钥          |
