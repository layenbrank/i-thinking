import { CACHE_TOKEN } from '@/utils/http/token.ts'
import type { HttpRequest } from '@ngify/http'
import { HttpResponse, type HttpEvent, type HttpHandlerFn } from '@ngify/http'
...
export function isCachable(request: HttpRequest<unknown>): boolean {
  return request.context.has(CACHE_TOKEN)
}
...
export class CacheService {
   ...
   * @param request
   */
  toRead(request: HttpRequest<unknown>): HttpResponse<unknown> | null {
...
export function cacheInterceptor(
  ...
  // 缓存命中则直接返回缓存的请求响应
  const response = cacheService.toRead(request)
